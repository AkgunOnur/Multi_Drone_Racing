...
class MultiGates_v1(BaseDrone_SelfPlay):
    ...
    def __init__(self,
        ...
        self.drone_collision = drone_collision
    ...
    def actionSpace(self, seed=None):
        act_lower_bound = np.array([[-0.05, -0.05, -0.05, -0.01] for i in range(self.NUM_DRONES)], dtype=np.float32)
        act_upper_bound = np.array([[0.05, 0.05, 0.05, 0.01] for i in range(self.NUM_DRONES)], dtype=np.float32)
    ...
    def reset(self,
        ...
        self.GATE_IDS = []
        if self.no_model:
            initial_xyzs = generate_initial_positions(self.all_agents, self.gate_positions, self.gate_rpy)
        else:
        ...
        if self.no_model:
            return agent_state, initial_info
    ...
    def _computeDroneFailed(self, drone_name):
        ...
        for j in range(len(self.gate_positions)):
            ...
            if collision and self.drone_collision:
                return True

        if self.drone_collision:
    ...
    def _computeDroneFailed_hard(self):
        distance_threshold = 5

        for drone_i in range(self.NUM_DRONES):
            if self.failed_drones[drone_i]:
                continue

            state = self._getDroneStateVector(drone_i)
            drone_i_pos = np.array(state[0:3])
            drone_i_rpy = np.array(state[7:10])

            if np.abs(drone_i_rpy[0]) > 3 * np.pi / 4 or np.abs(drone_i_rpy[1]) > 3 * np.pi / 4:
                self.failed_drones[drone_i] = 1
            else:
                # print ("self.GATE_IDS: ", self.GATE_IDS)
                for gate_i in range(len(self.gate_positions)):
                    # print ("ind: ", gate_i, " pos: ", self.gate_positions[gate_i]," gate_id: ", self.GATE_IDS[gate_i])
                    euclidean_dist = np.linalg.norm(drone_i_pos - self.gate_positions[gate_i])
                    collision = p.getContactPoints(bodyA=self.DRONE_IDS[drone_i], bodyB=self.GATE_IDS[gate_i], physicsClientId=self.CLIENT)

                    if euclidean_dist >= distance_threshold or collision:
                        self.failed_drones[drone_i] = 1
                        break

                if not self.failed_drones[drone_i]:
                    for drone_j in range(self.NUM_DRONES):
                        if drone_i == drone_j:
                            continue

                        if not self.failed_drones[drone_j]:
                            drone_collision = p.getContactPoints(bodyA=self.DRONE_IDS[drone_i], bodyB=self.DRONE_IDS[drone_j], physicsClientId=self.CLIENT)
                            if drone_collision:
                                self.failed_drones[drone_i] = 1
                                self.failed_drones[drone_j] = 1
                                break

        return sum(self.failed_drones.values()) == self.NUM_DRONES
    ...
    def step(self,
        ...
        if self.no_model:
            return clipped_action, agent_state, reward_val, done, truncated, {}
...
def generate_initial_positions(all_agents, gate_positions, gate_rpy, track_width=2.0, margin=0.5):
    ...
    for i, agent in enumerate(all_agents):
        ...
        gate_index = 0 #np.random.randint(len(gate_positions))
