...
from gym_pybullet_drones.control.MPC import MPCControl
from gym_pybullet_drones.utils.utils import record_video, GateNavigator


gate_folder = "/home/onur/Downloads/MultiDrone/sb3_selfplay/assets/gate.urdf"


class MultiGates_v3(BaseDrone_SelfPlay):
    ...
    def __init__(self,
        ...
        if act in [ActionType.PID, ActionType.VEL, ActionType.ONE_D_PID, ActionType.POS]:
            ...
            if drone_model in [DroneModel.CF2X, DroneModel.CF2P]:
                ...
                # self.ctrl = {agent: MPCControl(drone_model=DroneModel.CF2X) for agent in self.all_agents}
            else:
    ...
    def _preprocessAction(self,
        ...
        for agent_index, agent in enumerate(self.all_agents):
            ...
            if self.mode == "planner":
                ...
                target_pos = drone_pos + action_k[0:3]
                target_orient = euler
                target_orient[2] += action_k[3]
    ...
    def _computePenalty(self, drone_name):
        state_i = self._getDroneStateVector(drone_name)
        drone_i_pos = np.array(state_i[0:3])
        coeff = (self.difficulty_level + 1) / 2.0 
        ...
        for j in range(len(self.gate_positions)):
            ...
            if collision:
                penalty += -coeff * self.penalty_value
                # print (f"{self.timesteps} {drone_name} collision with the gate {j}! Penalty: {penalty}")
        ...
        for drone_i in self.all_agents:
            ...
            if drone_collision:
                penalty += -coeff * self.penalty_value
                # print (f"{self.timesteps} {drone_name} collision with the {drone_i}! Penalty: {penalty}")
            elif drone_distance < distance_threshold:
                penalty += -coeff* self.penalty_value / 2.0
                # print (f"{self.timesteps} {drone_name} is too close to {drone_i}! Penalty: {penalty}")

        return penalty
    
    
    # def calculate_team_reward(self, drones):
    #     TEAM_COMPLETION_BONUS = 2.0
    #     if all(self.navigators[agent].completed_laps > self.n_completion[agent] for agent in drones):
    ...
    def step(self,
        ...
        for agent in self.all_agents:
            ...
            if self._computeDroneFailed(agent):
                ...
                penalty = self._computePenalty(drone_name=agent)
                current_reward[agent] += penalty
