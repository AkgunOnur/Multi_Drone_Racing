...
class EvalCallback(EventCallback):
    ...
    def __init__(
        ...
        verbose: int = 1,
        warn: bool = True,
        opponent_model_update: bool = False
    ):
        ...
        self.render = render
        self.warn = warn
        self.opponent_model_update = opponent_model_update
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            mean_dumb_reward, std_dumb_reward = np.mean(episode_dumb_rewards), np.std(episode_dumb_rewards)
            mean_ep_length, std_ep_length = np.mean(episode_lengths), np.std(episode_lengths)
            self.last_mean_reward = float(mean_reward)

            if self.verbose >= 1:
                print(f"Eval num_timesteps={self.num_timesteps}, " f"episode_reward={mean_reward:.2f} +/- {std_reward:.2f}, ", f"dumb_reward={mean_dumb_reward:.2f} +/- {std_dumb_reward:.2f}")
                print(f"Episode length: {mean_ep_length:.2f} +/- {std_ep_length:.2f}")
            # Add to current Logger
            self.logger.record("eval/mean_reward", float(mean_reward))
            self.logger.record("eval/dumb_reward", float(mean_dumb_reward))
            ...
            if not self.opponent_model_update: 
                if mean_reward > self.best_mean_reward:
                    if self.verbose >= 1:
                        print(f"Agent reward: {mean_reward:.4f}. Model is saved!")
                    if self.best_model_save_path is not None:
                ...
                if mean_reward > self.best_mean_reward and mean_reward > mean_dumb_reward:
                    if self.verbose >= 1:
                        print(f"Prev opponent reward: {mean_dumb_reward:.4f} Agent reward: {mean_reward:.4f}. Model is saved!")
                    if self.best_model_save_path is not None:
                        self.model.save(os.path.join(self.best_model_save_path, "best_model"))
                    self.best_mean_reward = float(mean_reward)
        ...
        return continue_training, update_opponent
